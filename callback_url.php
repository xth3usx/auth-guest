...
// Verificar se o code_verifier foi armazenado na sessão ou passado como parâmetro na URL
if (isset($_SESSION['code_verifier'])) {
    $code_verifier = $_SESSION['code_verifier'];
} elseif (isset($_GET['code_verifier_param'])) {
    $code_verifier = $_GET['code_verifier_param'];  // Obtém o code_verifier da URL como fallback
} else {
    echo "Nenhum code_verifier encontrado.";
    exit();
}
...
function getAccessToken($code, $code_verifier) {
    ...
    // Exibe o código HTTP e a resposta completa para depuração
    if ($http_code !== 200) {
        echo "Erro ao trocar o código pelo token. Código HTTP: $http_code<br>";
